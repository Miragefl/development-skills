...
Run this spine for Java testing:

1. **定层**: 决定测什么层——单元（Service/逻辑，纯 mock 依赖）/ 集成（Controller+Repository，@SpringBootTest）/ DAO（Mapper 打真库或 H2）。不混层。
...
测试分层、TDD 细节、Mock 策略、Java 后端测试速查见 [TEST-STRATEGIES.md](./TEST-STRATEGIES.md)。
